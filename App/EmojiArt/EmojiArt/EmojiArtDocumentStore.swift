...
class EmojiArtDocumentStore: ObservableObject
{
    ...
    func setName(_ name: String, for document: EmojiArtDocument) {
        if let url = directory?.appendingPathComponent(name) {
            if !documentNames.values.contains(name) {
                ...
                print("Existe un archivo con ese nombre, no se permite renombrar")
            }
        } else {
            documentNames[document] = name
        }
    ...
    init(directory: URL) {
        ...
        do {
            ...
            print("EmojiArtDocumentStore: couldn't create sore from directory \(directory): \(error.localizedDescription)")
        }
    }
}
